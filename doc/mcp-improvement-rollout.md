## MCP Improvement Rollout Plan for c64bridge

Purpose: Execute the prioritized improvements captured in `doc/mcp-improvement-plan.md` so the MCP server delivers better grounding, creative defaults, and structured outputs. This rollout is written for an LLM operator; all tasks must be completed strictly in order with checkboxes ticked sequentially.

### Required Reading (complete before any checkbox work)

- `doc/mcp-improvement-plan.md` — Findings, recommendations, ranked shortlist.
...
- [ ] Confirm all documents in “Required Reading” are reviewed and summarize key acceptance criteria from `doc/mcp-improvement-plan.md` in your working notes.
