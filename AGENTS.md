...
- `mcp-manifest.json` (in the repository root) is auto-generated by `npm run manifest` (or `npm run build`). Clients read this manifest to discover available MCP tools. Regenerate it when tool decorators change instead of editing it directly.
