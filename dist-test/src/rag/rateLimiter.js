...
export class SlidingWindowRateLimiter {
    constructor(maxPerSecond, timeSource = new RealTimeSource()) {
        this.keyToTimestamps = new Map();
    ...
    pruneOld(key, now) {
        ...
        if (!arr)
            return;
        let startIdx = 0;
        while (startIdx < arr.length && arr[startIdx] < windowStart)
            startIdx++;
        if (startIdx > 0)
            arr.splice(0, startIdx);
    }
    /**
     * Wait until a token is available for the given key, then consume it.
     */
    async consume(key) {
        if (this.maxPerSecond === Infinity)
            return; // disabled
        if (this.maxPerSecond <= 0)
            throw new Error("maxPerSecond must be > 0 or Infinity");
...
export class AdaptiveRateLimiter {
    constructor(defaultRps, timeSource = new RealTimeSource(), opts = {}) {
        this.keyToTimestamps = new Map();
        this.keyToRps = new Map(); // [currentRps]
        this.keyToLastAdjust = new Map();
        if (!(defaultRps > 0))
            throw new Error("defaultRps must be > 0");
    ...
    maybeRecover(key, now) {
        const cur = this.currentRps(key);
        if (cur >= this.defaultRps)
            return;
