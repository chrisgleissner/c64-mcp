...
function generateCharsetQuickref() {
...
const BASE_RESOURCES = KNOWLEDGE_BUNDLES.flatMap((bundle, bundleIndex) => bundle.resources.map((resource, resourceIndex) => {
    const baseResource = {
        uri: resource.uri,
        name: resource.name,
...
const INDEX_RESOURCE = {
    ...
    buildContent: (resources) => renderKnowledgeIndex(resources),
};
const ALL_RESOURCES = [...BASE_RESOURCES, INDEX_RESOURCE];
function renderKnowledgeIndex(resources) {
    const sections = KNOWLEDGE_BUNDLES.map((bundle) => {
        const bundleResources = resources.filter((resource) => resource.metadata.bundle.id === bundle.id);
...
export function getKnowledgeResource(uri) {
    return ALL_RESOURCES.find((resource) => resource.uri === uri);
}
export function readKnowledgeResource(uri, projectRoot) {
    const resource = getKnowledgeResource(uri);
    if (!resource) {
        return undefined;
    }
    if (resource.buildContent) {
        const baseResources = ALL_RESOURCES.filter((item) => item.uri !== resource.uri);
        return {
            uri: resource.uri,
