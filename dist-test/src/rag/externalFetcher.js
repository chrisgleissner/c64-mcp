...
import { RealTimeSource, AdaptiveRateLimiter } from './rateLimiter.js';
import { parseUrlSafe, sameRegisteredDomain, getRegisteredDomain } from './urlUtils.js';
const execAsync = promisify(exec);
export async function readCsv(pathCsv) {
...
function normalizeGithubUrl(original) {
...
function isGithubFamily(hostname) {
    return GITHUB_HOSTS.has(hostname.toLowerCase());
}
function isAllowedDomain(seed, candidate) {
...
function shouldFollowLink(seed, current, next) {
...
function isGithubSingleFileUrl(url) {
...
function isBinaryContentType(ct) {
    ...
    if (t.includes('xml'))
        return false;
    if (t.includes('html'))
        return false; // we will parse links from HTML
    return true;
}
...
async function pruneNonSourceFiles(root, allowedExtensions) {
    let kept = 0;
    let removed = 0;
...
function parseFileEntry(fullPath) {
...
function detectMachineSuffix(base) {
...
async function cleanupC64SpecificFiles(root, log) {
    const removedPaths = new Set();
    const removeFile = async (entry, reason, extra) => {
...
async function removeDuplicateFiles(root, log) {
...
async function downloadGithubRepoZip(seedUrl, outDir, userAgent, log) {
    ...
    for (const branch of branches) {
        ...
        const response = await fetch(zipUrl, {
            ...
            log({ level: 'error', event: 'github_repo_error', data: { seed: seedUrl.toString(), message: err.message } });
            return { ok: false };
        });
...
async function defaultGithubRepoFetcher(args) {
...
async function writeGithubRepoMetadata(repoDir, payload) {
...
function buildLicenseAttribution(owner, repo, licenseId, licenseName) {
...
async function fetchGithubLicense(args) {
...
function parseGithubRepoInfo(url) {
...
export async function fetchFromCsv(opts) {
    ...
    for (const row of rows) {
        ...
        const depth = Number.isFinite(row.depth ?? NaN) ? row.depth : defaultDepth;
            ...
            while (true) {
                ...
                try {
                    ...
                    if (statusCode && statusCode >= 300 && statusCode < 400 && headers.location) {
                        ...
                        let redirected = null;
                        try {
                            redirected = new URL(loc, targetUrl);
                        }
                        catch { }
                        if (redirected) {
...
async function withRetries(requester, url, userAgent, maxRetries, onStatus) {
    let attempt = 0;
    ...
    while (true) {
        try {
            ...
            if (onStatus)
                await onStatus(res.statusCode);
            if (res.statusCode && (res.statusCode >= 500 || res.statusCode === 429)) {
                if (attempt >= maxRetries)
                    return res; // give up returning last response
