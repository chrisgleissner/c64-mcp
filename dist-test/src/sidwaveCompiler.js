 ...
 */
export function compileSidwaveToPrg(doc) {
    const system = (doc.song.mode ?? "PAL");
    ...
    const voices = [1, 2, 3].map((id) => doc.voices.find((v) => v.id === id) ?? { id, name: `Voice${id}`, waveform: "triangle", adsr: [2, 2, 10, 3], pulse_width: 2048, patterns: {} });
    const freqFrames = [new Uint16Array(cappedFrames), new Uint16Array(cappedFrames), new Uint16Array(cappedFrames)];
    const ctrlFrames = [new Uint8Array(cappedFrames), new Uint8Array(cappedFrames), new Uint8Array(cappedFrames)];
    const pwFrames = [new Uint16Array(cappedFrames), new Uint16Array(cappedFrames), new Uint16Array(cappedFrames)];
    const adFrames = [new Uint8Array(cappedFrames), new Uint8Array(cappedFrames), new Uint8Array(cappedFrames)];
    const srFrames = [new Uint8Array(cappedFrames), new Uint8Array(cappedFrames), new Uint8Array(cappedFrames)];
    // Build frames from timeline/patterns where possible
    const framesForVoice = [[], [], []];
    ...
    for (let vi = 0; vi < 3; vi += 1) {
        ...
        adFrames[vi].fill(ad);
        srFrames[vi].fill(sr);
        const waveformCtrl = waveformToCtrl(v.waveform ?? "pulse");
        const basePw = Math.max(0, Math.min(0x0fff, Math.floor(v.pulse_width ?? 0x0800)));
        ...
        for (const section of doc.timeline ?? []) {
            ...
            const pat = (patternName ? patterns[patternName] : undefined);
            if (!pat) {
                // Fill section with sustained default note
                for (let i = 0; i < totalFramesInSection; i += 1)
                    notesForSong.push("C3");
                continue;
            }
            if (pat.type === "arpeggio" && Array.isArray(pat.notes) && pat.notes.length) {
                ...
                for (let f = 0; f < totalFramesInSection; f += 1) {
                    if (f % stepFrames === 0)
                        mi = (mi + 1) % Math.max(1, intervals.length);
 ...
 */
export function compileSidwaveToSid(doc, prg, options) {
...
function hzToSidFrequency(hz, system) {
    const phi2 = system === "PAL" ? 985248 : 1022727;
    const value = Math.round((hz * 65536) / phi2);
    return Math.max(0, Math.min(0xffff, value));
}
function waveformToCtrl(w) {
    const wf = (w || "pulse").toLowerCase();
    if (wf.startsWith("tri"))
        return 1 << 4;
    if (wf.startsWith("saw"))
        return 1 << 5;
    if (wf.startsWith("pulse"))
        return 1 << 6;
    if (wf.startsWith("noise"))
        return 1 << 7;
    return 1 << 6;
}
function inferNotesForVoice(v) {
    const pnames = Object.keys(v.patterns ?? {});
    for (const name of pnames) {
        const p = v.patterns[name];
        if (!p)
            continue;
        if (Array.isArray(p.notes))
            return p.notes;
        if (Array.isArray(p.groove))
            return p.groove;
    }
    return undefined;
}
function inferVoiceFx(v) {
    const patterns = v.patterns ?? {};
    const first = Object.values(patterns)[0];
...
function pushByteRows(out, label, items, perLine = 16) {
    out.push("");
    out.push(`${label}:`);
