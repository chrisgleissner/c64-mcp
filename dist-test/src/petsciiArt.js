...
*/
import { findGlyphByBasicChar, getChargenGlyphs } from "./chargen.js";
const POPCOUNT = (() => {
...
export function bitmapToHexRows(bitmap) {
    const rows = [];
...
function wrapTextLines(source, maxCols, maxRows) {
    const lines = [];
...
function splitIntoBlocks(bitmap) {
...
function distanceBetween(block, glyph) {
    let score = 0;
    for (let i = 0; i < 8; i += 1) {
...
export function bitmapToPetsciiCodes(bitmap) {
...
export function createPetsciiArt(options) {
