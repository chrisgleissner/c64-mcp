...
function parseIntervals(pattern) {
...
function shouldAutoAnalyze(request) {
...
function resolveAnalyzer(ctx) {
    const candidate = ctx.client?.recordAndAnalyzeAudio;
    if (typeof candidate === "function") {
        return (options) => candidate.call(ctx.client, options);
    }
    return (options) => recordAndAnalyzeAudio(options);
}
function extractRmsMetrics(globalMetrics) {
...
function normaliseSidwaveInput(input) {
...
function generateAudioFeedback(analysisResult, userRequest) {
    try {
        ...
        voices.forEach((voice, index) => {
            ...
            const validNotes = notes.filter((n) => n?.note && n?.frequency);
            const voiceId = voice?.id ?? index + 1;
            if (validNotes.length > 0) {
                const preview = validNotes.slice(0, 5).map((n) => `${n.note}(${Math.round(n.frequency)}Hz)`);
        ...
        if (/(sound.*right|sound.*good|sound.*correct)/.test(userRequest.toLowerCase())) {
            const deviation = Math.abs(globalMetrics.average_pitch_deviation ?? 0);
            if (deviation < 20) {
        ...
        },
        {
            ...
            async execute(args, ctx) {
                try {
                    ...
                    const voice = Math.max(1, Math.min(3, Math.floor(parsed.voice ?? 1)));
        ...
        },
        {
            ...
            async execute(args, ctx) {
                try {
                    ...
                    const voice = Math.max(1, Math.min(3, Math.floor(parsed.voice)));
        ...
        },
        {
            ...
            async execute(args, ctx) {
                try {
                    ...
                    const executionError = new ToolExecutionError(error instanceof Error ? error.message : String(error), { cause: error instanceof Error ? error : undefined });
            ...
            async execute(args, ctx) {
                try {
                    ...
                    if (!shouldAutoAnalyze(parsed.request)) {
                        return textResult("No audio verification keywords detected. Use phrases like 'check the music' or 'verify the SID playback'.", {
                            analyzed: false,
