...
import { buildDescriptorIndex, ensureDescriptor, extendSchemaWithOp, createOperationHandlers, } from "./utils.js";
...
const graphicsOperations = [
    {
        op: "create_petscii",
        schema: extendSchemaWithOp("create_petscii", ensureDescriptor(graphicsDescriptorIndex, "create_petscii").inputSchema, { description: "Generate PETSCII art from prompts, text, or explicit bitmap data." }),
        handler: groupedGraphicsHandlers.create_petscii,
    },
    {
        op: "render_petscii",
        schema: extendSchemaWithOp("render_petscii", ensureDescriptor(graphicsDescriptorIndex, "render_petscii").inputSchema, { description: "Render PETSCII text with optional border/background colours." }),
        handler: groupedGraphicsHandlers.render_petscii,
    },
    {
        op: "generate_sprite",
        schema: extendSchemaWithOp("generate_sprite", ensureDescriptor(graphicsDescriptorIndex, "generate_sprite").inputSchema, { description: "Build and run a sprite PRG from raw 63-byte sprite data." }),
        handler: groupedGraphicsHandlers.generate_sprite,
    },
    {
        op: "generate_bitmap",
        schema: extendSchemaWithOp("generate_bitmap", {
            description: "Reserved high-resolution bitmap generator (coming soon).",
            type: "object",
            properties: {},
            additionalProperties: false,
        }, { description: "Reserved high-resolution bitmap generator (coming soon)." }),
        handler: async () => toolErrorResult(new ToolExecutionError("c64_graphics op generate_bitmap is not yet available", {
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_graphics", graphicsOperationHandlers),
        },
    ],
});
