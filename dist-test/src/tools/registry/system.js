...
const systemOperations = [
    {
        op: "pause",
        schema: extendSchemaWithOp("pause", ensureDescriptor(machineDescriptorIndex, "pause").inputSchema, { description: "Pause the machine using DMA halt until resumed." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(machineControlModule, "pause", rawArgs, ctx),
    },
    {
        op: "resume",
        schema: extendSchemaWithOp("resume", ensureDescriptor(machineDescriptorIndex, "resume").inputSchema, { description: "Resume CPU execution after a DMA pause." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(machineControlModule, "resume", rawArgs, ctx),
    },
    {
        op: "reset",
        schema: extendSchemaWithOp("reset", ensureDescriptor(machineDescriptorIndex, "reset_c64").inputSchema, { description: "Issue a soft reset without cutting power." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(machineControlModule, "reset_c64", rawArgs, ctx),
    },
    {
        op: "reboot",
        schema: extendSchemaWithOp("reboot", ensureDescriptor(machineDescriptorIndex, "reboot_c64").inputSchema, { description: "Trigger a firmware reboot to recover from faults." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(machineControlModule, "reboot_c64", rawArgs, ctx),
    },
    {
        op: "poweroff",
        schema: extendSchemaWithOp("poweroff", ensureDescriptor(machineDescriptorIndex, "poweroff").inputSchema, { description: "Request a controlled shutdown via the Ultimate firmware." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(machineControlModule, "poweroff", rawArgs, ctx),
    },
    {
        op: "menu",
        schema: extendSchemaWithOp("menu", ensureDescriptor(machineDescriptorIndex, "menu_button").inputSchema, { description: "Toggle the Ultimate menu button for navigation." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(machineControlModule, "menu_button", rawArgs, ctx),
    },
    {
        op: "start_task",
        schema: extendSchemaWithOp("start_task", ensureDescriptor(metaDescriptorIndex, "start_background_task").inputSchema, { description: "Start a named background task that runs on an interval." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(metaModule, "start_background_task", rawArgs, ctx),
    },
    {
        op: "stop_task",
        schema: extendSchemaWithOp("stop_task", ensureDescriptor(metaDescriptorIndex, "stop_background_task").inputSchema, { description: "Stop a specific background task and clear its timer." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(metaModule, "stop_background_task", rawArgs, ctx),
    },
    {
        op: "stop_all_tasks",
        schema: extendSchemaWithOp("stop_all_tasks", ensureDescriptor(metaDescriptorIndex, "stop_all_background_tasks").inputSchema, { description: "Stop every running background task and persist state." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(metaModule, "stop_all_background_tasks", rawArgs, ctx),
    },
    {
        op: "list_tasks",
        schema: extendSchemaWithOp("list_tasks", ensureDescriptor(metaDescriptorIndex, "list_background_tasks").inputSchema, { description: "List known background tasks with status metadata." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_system", systemOperationHandlers),
        },
    ],
});
