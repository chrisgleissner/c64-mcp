...
const streamOperations = [
    {
        op: "start",
        schema: extendSchemaWithOp("start", ensureDescriptor(streamingDescriptorIndex, "stream_start").inputSchema, { description: "Start an Ultimate streaming session toward a host:port target." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(streamingModule, "stream_start", rawArgs, ctx),
    },
    {
        op: "stop",
        schema: extendSchemaWithOp("stop", ensureDescriptor(streamingDescriptorIndex, "stream_stop").inputSchema, { description: "Stop an active Ultimate streaming session." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_stream", streamOperationHandlers),
        },
    ],
});
