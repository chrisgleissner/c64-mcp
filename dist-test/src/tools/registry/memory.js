...
const memoryOperations = [
    {
        op: "read",
        schema: extendSchemaWithOp("read", ensureDescriptor(memoryDescriptorIndex, "read").inputSchema, { description: "Read a range of bytes and return a hex dump with address metadata." }),
        handler: groupedMemoryHandlers.read,
    },
    {
        op: "write",
        schema: extendSchemaWithOp("write", ensureDescriptor(memoryDescriptorIndex, "write").inputSchema, { description: "Write a hexadecimal byte sequence into RAM." }),
        handler: groupedMemoryHandlers.write,
    },
    {
        op: "read_screen",
        schema: extendSchemaWithOp("read_screen", ensureDescriptor(memoryDescriptorIndex, "read_screen").inputSchema, { description: "Return the current 40x25 text screen converted to ASCII." }),
        handler: groupedMemoryHandlers.read_screen,
    },
    {
        op: "wait_for_text",
        schema: extendSchemaWithOp("wait_for_text", ensureDescriptor(metaDescriptorIndex, "wait_for_screen_text").inputSchema, { description: "Poll the screen until a substring or regex appears, or timeout elapses." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_memory", memoryOperationHandlers),
        },
    ],
});
