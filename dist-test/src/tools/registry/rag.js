...
const ragOperations = [
    {
        op: "basic",
        schema: extendSchemaWithOp("basic", ensureDescriptor(ragDescriptorIndex, "rag_retrieve_basic").inputSchema, { description: "Retrieve BASIC references and snippets from the local knowledge base." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(ragModule, "rag_retrieve_basic", rawArgs, ctx),
    },
    {
        op: "asm",
        schema: extendSchemaWithOp("asm", ensureDescriptor(ragDescriptorIndex, "rag_retrieve_asm").inputSchema, { description: "Retrieve 6502/6510 assembly references from the local knowledge base." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_rag", ragOperationHandlers),
        },
    ],
});
