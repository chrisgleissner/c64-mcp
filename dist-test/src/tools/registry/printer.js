...
const printerOperations = [
    {
        op: "print_text",
        schema: extendSchemaWithOp("print_text", ensureDescriptor(printerDescriptorIndex, "print_text").inputSchema, { description: "Generate BASIC that prints text to device 4." }),
        handler: async (rawArgs, ctx) => invokePrintTool("print_text", rawArgs, ctx),
    },
    {
        op: "print_bitmap",
        schema: extendSchemaWithOp("print_bitmap", printBitmapArgsSchema.jsonSchema, { description: "Print a bitmap row via Commodore (BIM) or Epson ESC/P workflows." }),
    ...
    {
        op: "define_chars",
        schema: extendSchemaWithOp("define_chars", ensureDescriptor(printerDescriptorIndex, "define_printer_chars").inputSchema, { description: "Define custom printer characters (Commodore DLL mode)." }),
...
function invokePrintTool(toolName, rawArgs, ctx) {
    const { [OPERATION_DISCRIMINATOR]: _ignored, ...rest } = rawArgs;
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_printer", printerOperationHandlers),
        },
    ],
});
