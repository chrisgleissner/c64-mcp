...
const configSnapshotArgsSchema = objectSchema({
...
const configRestoreArgsSchema = objectSchema({
...
const configDiffArgsSchema = objectSchema({
...
const configOperations = [
    {
        op: "list",
        schema: extendSchemaWithOp("list", ensureDescriptor(developerDescriptorIndex, "config_list").inputSchema, { description: "List configuration categories reported by the firmware." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_list", rawArgs, ctx),
    },
    {
        op: "get",
        schema: extendSchemaWithOp("get", ensureDescriptor(developerDescriptorIndex, "config_get").inputSchema, { description: "Read a configuration category or specific item." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_get", rawArgs, ctx),
    },
    {
        op: "set",
        schema: extendSchemaWithOp("set", ensureDescriptor(developerDescriptorIndex, "config_set").inputSchema, { description: "Write a configuration value in the selected category." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_set", rawArgs, ctx),
    },
    {
        op: "batch_update",
        schema: extendSchemaWithOp("batch_update", ensureDescriptor(developerDescriptorIndex, "config_batch_update").inputSchema, { description: "Apply multiple configuration updates in a single request." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_batch_update", rawArgs, ctx),
    },
    {
        op: "load_flash",
        schema: extendSchemaWithOp("load_flash", ensureDescriptor(developerDescriptorIndex, "config_load_from_flash").inputSchema, { description: "Load configuration from flash storage." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_load_from_flash", rawArgs, ctx),
    },
    {
        op: "save_flash",
        schema: extendSchemaWithOp("save_flash", ensureDescriptor(developerDescriptorIndex, "config_save_to_flash").inputSchema, { description: "Persist the current configuration to flash storage." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_save_to_flash", rawArgs, ctx),
    },
    {
        op: "reset_defaults",
        schema: extendSchemaWithOp("reset_defaults", ensureDescriptor(developerDescriptorIndex, "config_reset_to_default").inputSchema, { description: "Reset firmware configuration to factory defaults." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "config_reset_to_default", rawArgs, ctx),
    },
    {
        op: "read_debugreg",
        schema: extendSchemaWithOp("read_debugreg", ensureDescriptor(developerDescriptorIndex, "debugreg_read").inputSchema, { description: "Read the Ultimate debug register ($D7FF)." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "debugreg_read", rawArgs, ctx),
    },
    {
        op: "write_debugreg",
        schema: extendSchemaWithOp("write_debugreg", ensureDescriptor(developerDescriptorIndex, "debugreg_write").inputSchema, { description: "Write a hex value to the Ultimate debug register ($D7FF)." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "debugreg_write", rawArgs, ctx),
    },
    {
        op: "info",
        schema: extendSchemaWithOp("info", ensureDescriptor(developerDescriptorIndex, "info").inputSchema, { description: "Retrieve Ultimate hardware information and status." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "info", rawArgs, ctx),
    },
    {
        op: "version",
        schema: extendSchemaWithOp("version", ensureDescriptor(developerDescriptorIndex, "version").inputSchema, { description: "Fetch firmware version details." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(developerModule, "version", rawArgs, ctx),
    },
    {
        op: "snapshot",
        schema: extendSchemaWithOp("snapshot", configSnapshotArgsSchema.jsonSchema, { description: "Snapshot configuration to disk for later restore or diff." }),
    ...
    {
        op: "restore",
        schema: extendSchemaWithOp("restore", configRestoreArgsSchema.jsonSchema, { description: "Restore configuration from a snapshot file." }),
        handler: async (rawArgs, ctx) => {
            const { [OPERATION_DISCRIMINATOR]: _ignored, ...rest } = rawArgs;
    ...
    {
        op: "diff",
        schema: extendSchemaWithOp("diff", configDiffArgsSchema.jsonSchema, { description: "Compare the current configuration with a snapshot." }),
        handler: async (rawArgs, ctx) => {
            const { [OPERATION_DISCRIMINATOR]: _ignored, ...rest } = rawArgs;
    ...
    {
        op: "shuffle",
        schema: extendSchemaWithOp("shuffle", ensureDescriptor(metaDescriptorIndex, "program_shuffle").inputSchema, { description: "Discover PRG/CRT files and run each with optional screen capture." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_config", configOperationHandlers),
        },
    ],
});
