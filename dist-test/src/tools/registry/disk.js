...
const diskMountArgsSchema = objectSchema({
...
const createImageArgsSchema = objectSchema({
...
const printBitmapArgsSchema = objectSchema({
...
const diskOperations = [
    {
        op: "list_drives",
        schema: extendSchemaWithOp("list_drives", ensureDescriptor(storageDescriptorIndex, "drives_list").inputSchema, { description: "List Ultimate drive slots and their mounted images." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(storageModule, "drives_list", rawArgs, ctx),
    },
    {
        op: "mount",
        schema: extendSchemaWithOp("mount", diskMountArgsSchema.jsonSchema, { description: "Mount a disk image with optional verification and retries." }),
        handler: async (rawArgs, ctx) => {
            ...
            const typedAttachment = attachmentMode;
            const typedDriveMode = driveMode;
            if (verify) {
    ...
    {
        op: "unmount",
        schema: extendSchemaWithOp("unmount", ensureDescriptor(storageDescriptorIndex, "drive_remove").inputSchema, { description: "Remove the mounted image from an Ultimate drive slot." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(storageModule, "drive_remove", rawArgs, ctx),
    },
    {
        op: "file_info",
        schema: extendSchemaWithOp("file_info", ensureDescriptor(storageDescriptorIndex, "file_info").inputSchema, { description: "Inspect metadata for a file on the Ultimate filesystem." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(storageModule, "file_info", rawArgs, ctx),
    },
    {
        op: "create_image",
        schema: extendSchemaWithOp("create_image", createImageArgsSchema.jsonSchema, { description: "Create a blank disk image of the specified format." }),
        handler: async (rawArgs, ctx) => {
            const { [OPERATION_DISCRIMINATOR]: _ignored, ...rest } = rawArgs;
    ...
    {
        op: "find_and_run",
        schema: extendSchemaWithOp("find_and_run", ensureDescriptor(metaDescriptorIndex, "find_and_run_program_by_name").inputSchema, { description: "Search for a PRG/CRT by name substring and run the first match." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_disk", diskOperationHandlers),
        },
    ],
});
