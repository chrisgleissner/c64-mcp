...
const programOperations = [
    {
        op: "load_prg",
        schema: extendSchemaWithOp("load_prg", ensureDescriptor(programDescriptorIndex, "load_prg").inputSchema, { description: "Load a PRG from Ultimate storage without executing it." }),
        handler: groupedProgramHandlers.load_prg,
    },
    {
        op: "run_prg",
        schema: extendSchemaWithOp("run_prg", ensureDescriptor(programDescriptorIndex, "run_prg").inputSchema, { description: "Load and execute a PRG located on the Ultimate filesystem." }),
        handler: groupedProgramHandlers.run_prg,
    },
    {
        op: "run_crt",
        schema: extendSchemaWithOp("run_crt", ensureDescriptor(programDescriptorIndex, "run_crt").inputSchema, { description: "Mount and run a CRT cartridge image." }),
        handler: groupedProgramHandlers.run_crt,
    },
    {
        op: "upload_run_basic",
        schema: extendSchemaWithOp("upload_run_basic", ensureDescriptor(programDescriptorIndex, "upload_run_basic").inputSchema, { description: "Upload Commodore BASIC v2 source and execute it immediately." }),
        handler: groupedProgramHandlers.upload_run_basic,
    },
    {
        op: "upload_run_asm",
        schema: extendSchemaWithOp("upload_run_asm", ensureDescriptor(programDescriptorIndex, "upload_run_asm").inputSchema, { description: "Assemble 6502/6510 source, upload the PRG, and execute it." }),
        handler: groupedProgramHandlers.upload_run_asm,
    },
    {
        op: "batch_run",
        schema: extendSchemaWithOp("batch_run", ensureDescriptor(metaDescriptorIndex, "batch_run_with_assertions").inputSchema, { description: "Run multiple PRG/CRT programs with post-run assertions." }),
        handler: async (rawArgs, ctx) => invokeModuleTool(metaModule, "batch_run_with_assertions", rawArgs, ctx),
    },
    {
        op: "bundle_run",
        schema: extendSchemaWithOp("bundle_run", ensureDescriptor(metaDescriptorIndex, "bundle_run_artifacts").inputSchema, { description: "Capture screen, memory, and debug registers into an artifact bundle." }),
    ...
    tools: [
        {
            ...
            execute: createOperationDispatcher("c64_program", programOperationHandlers),
        },
    ],
});
