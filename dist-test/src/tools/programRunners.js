...
import { ToolExecutionError, ToolError, ToolValidationError, toolErrorResult, unknownErrorResult, } from "./errors.js";
import { pollForProgramOutcome } from "./pollValidator.js";
function extractFailureDetails(details) {
...
function parseBasicRuntimeErrors(screen) {
    const errors = [];
    const seen = new Set();
...
function attemptAutoFixBasicProgram(program, errors) {
    if (errors.length === 0) {
        return undefined;
    }
...
function normalizeRuntimeErrors(errors) {
...
function structuredExecutionError(message, data) {
    return {
        content: [
            {
                type: "text",
...
async function executeUploadRunBasic(rawArgs, ctx) {
    try {
        ...
        const runBasic = async (source) => ctx.client.uploadAndRunBasic(source);
        ...
        if (screenOutput) {
            ...
            if (errors.length > 0) {
                ...
                if (!retryResult.success) {
                    ...
                    return structuredExecutionError("Auto-fix failed due to firmware error while re-running BASIC program.", data);
                }
                let retryScreen;
                try {
                ...
                if (remainingErrors.length > 0) {
                    ...
                    return structuredExecutionError("BASIC program still reports errors after auto-fix attempt.", data);
                }
        ...
        if (shouldVerify) {
            try {
                ...
                if (outcome.status === "error") {
                    ...
                    return toolErrorResult(new ToolExecutionError("BASIC program verification failed", {
                        details: {
                            ...(outcome.message ? { message: outcome.message } : {}),
                            ...(outcome.line !== undefined ? { line: outcome.line } : {}),
                        },
                ...
                return toolErrorResult(new ToolExecutionError("Failed to verify BASIC program execution", {
...
async function executeUploadRunAsm(rawArgs, ctx) {
    try {
        ...
        if (!result.success) {
            return toolErrorResult(new ToolExecutionError("C64 firmware reported failure while running assembly program", {
                details: extractFailureDetails(result.details),
            }));
        }
        ...
        try {
            ...
            if (outcome.status === "crashed") {
                ...
                return toolErrorResult(new ToolExecutionError("Assembly program appears to have crashed (no screen changes detected)", {
            ...
            if (shouldVerify) {
                return toolErrorResult(new ToolExecutionError("Failed to verify assembly program execution", {
...
async function executeLoadPrg(rawArgs, ctx) {
    try {
...
async function executeRunPrg(rawArgs, ctx) {
    try {
        const parsed = prgFileArgsSchema.parse(rawArgs ?? {});
...
async function executeRunCrt(rawArgs, ctx) {
    try {
...
export const programOperationHandlers = {
