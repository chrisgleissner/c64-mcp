...
import { objectSchema, optionalSchema, stringSchema, } from "./schema.js";
...
const itemSchema = optionalSchema(stringSchema({
        ...
        },
        {
            ...
            async execute(args, ctx) {
                try {
                    ...
                    const result = await ctx.client.configSet(parsed.category, parsed.item, String(parsed.value));
                    if (!result.success) {
        ...
        },
        {
            ...
            async execute(args, ctx) {
                try {
                    ...
                    const requestPayload = Object.fromEntries(Object.entries(payload).map(([category, entries]) => [
                        category,
...
function normalizeCategoryListing(details) {
    if (details &&
