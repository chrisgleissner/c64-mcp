...
async function silenceSid(context) {
    const { client } = context;
    if (!(client && typeof client.sidSilenceAll === "function")) {
        throw new ToolExecutionError("sidSilenceAll is not available on this client");
    }
    const response = await client.sidSilenceAll();
    if (!response?.success) {
...
function resolveAnalyzer(context) {
    const { client } = context;
    if (client && typeof client.recordAndAnalyzeAudio === "function") {
        return (args) => client.recordAndAnalyzeAudio(args);
    }
    return recordAndAnalyzeAudio;
}
function normalizeSidwaveInput(input) {
...
async function performSilenceCheck(context, analyzer, options) {
    ...
    const metrics = extractRmsMetrics((analysis?.analysis?.global_metrics ?? {}));
...
export const tools = [
    {
        ...
        description: "Silence all SID voices, capture a short sample, and ensure the output is below an RMS threshold.",
    ...
    },
    {
        ...
        async execute(args, context) {
            try {
                ...
                const format = (parsed.output ?? "prg");
                ...
                const analysisParams = { durationSeconds: analysisDurationSeconds };
                if (expectedSidwave) {
                    analysisParams.expectedSidwave = expectedSidwave;
                }
                const analysis = await analyzer(analysisParams);
                const analysisMetrics = extractRmsMetrics((analysis?.analysis?.global_metrics ?? {}));
                if (analysisMetrics.average === null && analysisMetrics.max === null) {
                    throw new ToolExecutionError("Audio analysis did not include RMS metrics", {
                        details: { globalMetrics: analysis?.analysis?.global_metrics ?? {} },
                    });
                }
                let postSilence = null;
