...
const programShuffleArgsSchema = objectSchema({
    ...
    properties: {
        ...
        extensions: optionalSchema(arraySchema(stringSchema({ description: "File extensions to include (without dot)", minLength: 1 })), ["prg", "crt"]),
...
export const tools = [
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                const extensions = (parsed.extensions ?? ["prg", "crt"]);
                ...
                for (const ext of extensions) {
                    ...
                    try {
                        ...
                        const paths = Array.isArray(info) ? info : (Array.isArray(info?.paths) ? info.paths : []);
                ...
                for (const programPath of programs.slice(0, maxPrograms)) {
                    ...
                    try {
                        ...
                        if (ext === "prg") {
                            await ctx.client.runPrgFile(programPath);
                        }
                        else if (ext === "crt") {
                            await ctx.client.runCrtFile(programPath);
                        }
            ...
            try {
                ...
                for (const program of programs) {
                    ...
                    try {
                        ...
                        if (ext === "prg") {
                            await ctx.client.runPrgFile(program.path);
                        }
                        else if (ext === "crt") {
                            await ctx.client.runCrtFile(program.path);
                        }
                        await sleep(durationMs);
                        // Check assertions
                        ...
                        for (const assertion of assertions) {
                            if (assertion.type === "screen_contains") {
                                ...
                                const result = await ctx.client.readMemory(addr, "1");
                                ...
                                const result = await ctx.client.readMemory("$D404", "1");
