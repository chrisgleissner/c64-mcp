...
function extractPaths(value) {
    ...
    while (stack.length > 0) {
        ...
        if (typeof current === "object") {
            const maybeObj = current;
            if (typeof maybeObj.path === "string")
                acc.push(maybeObj.path);
            if (Array.isArray(maybeObj.paths))
                stack.push(maybeObj.paths);
            if (Array.isArray(maybeObj.entries))
                stack.push(maybeObj.entries);
        }
    }
    return acc;
}
...
function normalisePath(path, caseInsensitive) {
    ...
    try {
        ...
        if (parsed && typeof parsed === "object") {
            ...
            for (const rawEntry of rawRecent) {
                ...
                recent.push({
                    root: rootValue,
                    pattern: String(rawEntry.pattern),
...
async function persistState(statePath, entry, lastRunPath) {
...
function detectContainerFlag(record, path) {
    if (record.isContainer === true || record.is_container === true)
        return true;
    const typeValue = typeof record.type === "string" ? record.type.toLowerCase() : undefined;
    if (typeValue && CONTAINER_EXTENSIONS.has(typeValue))
        return true;
    const formatValue = typeof record.format === "string" ? record.format.toLowerCase() : undefined;
    if (formatValue && CONTAINER_EXTENSIONS.has(formatValue))
        return true;
    const ext = extractExtension(path);
    return CONTAINER_EXTENSIONS.has(ext);
}
function isDirectoryRecord(record, path) {
    if (record.isDirectory === true || record.directory === true)
        return true;
    const typeValue = typeof record.type === "string" ? record.type.toLowerCase() : undefined;
    if (typeValue === "directory" || typeValue === "dir" || typeValue === "folder")
        return true;
    if (typeof record.kind === "string") {
        const kindValue = record.kind.toLowerCase();
        if (kindValue === "directory" || kindValue === "folder")
            return true;
    }
    if (path.endsWith("/"))
        return true;
...
function extractEntriesFromInfo(input) {
    const map = new Map();
    const stack = [input];
    while (stack.length > 0) {
        const current = stack.pop();
        ...
        if (typeof current === "object") {
            ...
            if (!pathValue)
                continue;
            if (isDirectoryRecord(obj, pathValue))
                continue;
...
function createExtensionAccumulator() {
    return { ...createStatsAccumulator(), samples: [] };
}
function recordSize(acc, size) {
    acc.count += 1;
    if (size === null)
        return;
    acc.withSize += 1;
...
function recordExtension(acc, size, path, sampleLimit) {
    recordSize(acc, size);
    if (acc.samples.length < sampleLimit)
        acc.samples.push(path);
}
function deriveMean(acc) {
    if (acc.withSize === 0)
        return null;
    return acc.totalBytes / acc.withSize;
}
function toSummaryArray(map, keyName, options) {
    ...
    for (const [key, acc] of map.entries()) {
        ...
        if (includeSamples && "samples" in acc && Array.isArray(acc.samples)) {
            base.samples = acc.samples.slice(0, sampleLimit);
        }
        entries.push(base);
    }
    return entries;
}
function resolveHostFolder(path) {
    const hashIndex = path.indexOf("#");
    const hostPart = hashIndex === -1 ? path : path.slice(0, hashIndex);
    const lastSlash = hostPart.lastIndexOf("/");
    if (lastSlash <= 0)
        return hostPart.startsWith("/") ? "/" : hostPart || ".";
    return hostPart.slice(0, lastSlash);
}
function resolveContainerPath(path) {
    const hashIndex = path.indexOf("#");
    if (hashIndex === -1)
        return null;
...
function buildSearchPattern(root, extension) {
...
export const tools = [
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                const needle = parsed.caseInsensitive ? parsed.nameContains.toLowerCase() : parsed.nameContains;
                const exts = (parsed.extensions ?? []);
                const patterns = exts.length > 0
                    ? exts.map((e) => `${root}/**/*${parsed.nameContains}*.${e}`)
                    : [`${root}/**/*${parsed.nameContains}*`];
                const seen = new Set();
                const results = [];
                for (const pattern of patterns) {
                    const info = await ctx.client.filesInfo(pattern);
                    if (Array.isArray(info)) {
                        for (const p of info) {
                            ...
                            if (name.includes(needle) && !seen.has(p)) {
                                seen.add(p);
                                results.push(p);
                                if (results.length >= (parsed.maxResults ?? 50))
                                    break;
                            }
                        }
                    }
                    else if (info && typeof info === "object" && Array.isArray(info.paths)) {
    ...
    },
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                const root = parsed.root ?? "/";
                const extensionList = (parsed.extensions ?? []).map((ext) => ext.replace(/^\./, "").toLowerCase());
                    ...
                    try {
                        ...
                        for (const entry of extracted) {
                            ...
                            if (!existing) {
                                ...
                                if (existing.size === null && entry.size !== null)
                ...
                for (const entry of allEntries) {
                    ...
                    if (filterApplied && !filterSet.has(rawExtension))
                        continue;
                    const isContainerHost = isContainer && !path.includes("#");
                    if (isContainerHost && !includeContainers)
                        continue;
                ...
                extensionStats.sort((a, b) => {
                    const bytesA = a.totalBytes ?? 0;
                    const bytesB = b.totalBytes ?? 0;
                    if (bytesA !== bytesB)
    ...
    },
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                const extensions = ((parsed.extensions && parsed.extensions.length > 0) ? parsed.extensions : ["prg", "crt"]);
                const needle = caseInsensitive ? parsed.nameContains.toLowerCase() : parsed.nameContains;
                const seen = new Set();
                const candidates = [];
                for (const ext of extensions) {
                    ...
                    try {
                        ...
                        for (const raw of paths) {
                            ...
                            if (extensions.length > 0 && !extensions.some((e) => e.toLowerCase() === extCandidate))
                                continue;
                            seen.add(raw);
                            candidates.push(raw);
                        }
                ...
                let result;
                if (extension === "crt") {
                    result = await ctx.client.runCrtFile(matchedPath);
                }
                else if (extension === "prg") {
                    result = await ctx.client.runPrgFile(matchedPath);
                }
                else {
    ...
    },
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                if (powerOnIfNeeded) {
                    try {
                        const drives = await ctx.client.drivesList();
                        const targetDrive = Array.isArray(drives) ? drives.find((d) => d.id === drive) : null;
                        if (!targetDrive) {
                        ...
                        if (targetDrive.power === "off" || targetDrive.power === "OFF") {
                            const powerOn = await ctx.client.driveOn(drive);
                ...
                for (let attempt = 0; attempt <= maxRetries && !mountSuccess; attempt += 1) {
                    ...
                    try {
                        ...
                        const mount = await ctx.client.driveMount(drive, imagePath, mountOptions);
                        if (mount.success) {
                            mountSuccess = true;
                ...
                if (resetAfterMount) {
                    try {
                        const reset = await ctx.client.driveReset(drive);
