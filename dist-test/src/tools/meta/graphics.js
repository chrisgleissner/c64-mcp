...
function analyzeSprite(bytes, startAddress) {
    if (bytes.length !== 63)
        return null;
    let nonZeroRows = 0;
    let totalSetBits = 0;
    ...
    for (let row = 0; row < 21; row += 1) {
        ...
        if (firstBit !== null && firstBit < minX)
            minX = firstBit;
        if (lastBit !== null && lastBit > maxX)
            maxX = lastBit;
...
function analyzeCharset(bytes, startAddress) {
...
export const tools = [
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                try {
                    const read = await ctx.client.readMemory(`$${formatAddressHex(startAddress)}`, String(length));
                    ...
                    for (let offset = 0; offset + 63 <= length && sprites.length < maxSprites; offset += stride) {
                        ...
                        candidates += 1;
                        if (!window.some((b) => b !== 0))
                            continue;
                        const analysis = analyzeSprite(window, (startAddress + offset) & 0xFFFF);
                        if (!analysis)
                            continue;
                        if (analysis.nonZeroRows < minNonZeroRows)
                            continue;
                        if (analysis.totalSetBits < minSetBits)
                            continue;
                        if (includeBase64) {
            ...
            try {
                ...
                try {
                    ...
                    for (const startAddress of scanLocations) {
                        if (startAddress + charsetSize - 1 > 0xFFFF)
                            continue;
                        const read = await ctx.client.readMemory(`$${formatAddressHex(startAddress)}`, String(charsetSize));
                        if (!read.success || typeof read.data !== "string") {
                            continue; // Skip this location if read fails
                        }
                        const bytes = hexToBytes(read.data);
                        if (bytes.length < charsetSize)
                            continue;
