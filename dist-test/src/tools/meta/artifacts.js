...
export const tools = [
    {
        ...
        async execute(args, ctx) {
            try {
                ...
                const memoryRanges = (parsed.memoryRanges ?? []);
                ...
                if (memoryRanges.length > 0) {
                    ...
                    for (let i = 0; i < memoryRanges.length; i++) {
                        ...
                        const result = await ctx.client.readMemory(range.address, String(range.length));
                ...
                // Capture debugreg
                if (captureDebugReg) {
                    const debugreg = await ctx.client.debugregRead();
