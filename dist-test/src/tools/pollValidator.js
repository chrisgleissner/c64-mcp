 ...
 */
export function loadPollConfig() {
 ...
 */
function extractBasicError(screen) {
 ...
 */
async function pollBasicOutcome(client, logger, config) {
 ...
 */
async function pollAsmOutcome(client, logger, config) {
    const startTime = Date.now();
    let pollCount = 0;
    let runDetected = false;
    ...
    let prevIoSignature = null;
    let prevJiffyClock = null;
    let alive = false;
 ...
 */
export async function pollForProgramOutcome(type, client, logger, config) {
