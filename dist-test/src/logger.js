...
export function loggerFor(prefix) {
...
function createPrefixedLogger(prefix) {
    const render = (level, message, details) => {
        if (shouldSkip(level)) {
            return;
        }
...
export function formatPayloadForDebug(payload) {
    if (payload === null || payload === undefined) {
        return payload;
    }
    const buf = asBuffer(payload);
