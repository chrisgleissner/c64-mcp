...
function assemblyCoreSegment(hardware) {
    const focus = (() => {
        switch (hardware) {
            case "sid":
...
function graphicsCoreSegment(mode) {
    const intro = (() => {
        switch (mode) {
            case "text":
...
function printerCoreSegment(printerType) {
    const detail = (() => {
        switch (printerType) {
            case "commodore":
...
function mergeUniqueStrings(base, extras) {
...
function prepareAssemblyArgs(args) {
    ...
    const allowed = ["sid", "vic", "cia", "multi"];
    if (!allowed.includes(normalised)) {
        throw new Error(`assembly-program prompt does not support hardware "${hardware}". Expected one of: ${allowed.join(", ")}`);
    }
    return { hardware: normalised };
}
function prepareGraphicsArgs(args) {
    ...
    const allowed = ["text", "multicolour", "bitmap", "sprite"];
    if (!allowed.includes(normalised)) {
        throw new Error(`graphics-demo prompt does not support mode "${mode}". Expected one of: ${allowed.join(", ")}`);
    }
    return { mode: normalised };
}
function preparePrinterArgs(args) {
    ...
    const allowed = ["commodore", "epson"];
    if (!allowed.includes(normalised)) {
        throw new Error(`printer-job prompt does not support printerType "${printerType}". Expected one of: ${allowed.join(", ")}`);
    }
    return { printerType: normalised };
}
export function createPromptRegistry() {
    const knowledgeByUri = new Map(listKnowledgeResources().map((resource) => [resource.uri, resource]));
    const toolByName = new Map(toolRegistry.list().map((tool) => [tool.name, tool]));
    const definitions = [
        {
            descriptor: {
                name: "basic-program",
    ...
    const definitionByName = new Map(definitions.map((def) => [def.descriptor.name, def]));
    return {
        list() {
        ...
        resolve(name, args) {
            ...
            const toolNames = mergeUniqueStrings(definition.descriptor.tools, definition.selectTools ? definition.selectTools(prepared) : undefined);
