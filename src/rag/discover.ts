...
type CsvRow = { type: string; description: string; link: string; depth: number };
...
async function loadConfig(): Promise<DiscoverConfig> {
...
function normalizeRepoUrl(htmlUrl: string): string {
  try {
    ...
    if (u.hostname.toLowerCase() !== 'github.com') return htmlUrl.replace(/\/$/, '');
...
function classifyType(matchedExts: Set<string>): 'basic' | 'assembly' | 'misc' {
  if (matchedExts.has('.bas')) return 'basic';
  for (const ext of matchedExts) {
    if (ASSEMBLY_EXTS.has(ext)) return 'assembly';
  }
  return 'misc';
}
...
async function maybeWaitOnRateLimit(headers: Record<string, string | number | undefined>): Promise<void> {
...
async function run(): Promise<void> {
  ...
  const existingLinks = new Set(
    existingRows.map((r) => normalizeRepoUrl(r.link))
  );
  ...
  let repos: Map<string, RepoAgg> = new Map(); // key: owner/repo lowercased
  ...
  if (cache) {
        ...
        while (true) {
          ...
          try {
            ...
            if (status === 403 || status === 429) {
              const reset = Number((headers as any)['x-ratelimit-reset'] ?? 0);
              if (retryAfter > 0) {
          ...
          await maybeWaitOnRateLimit(headers as any);

          for (const item of data.items) {
            // Each code result item includes a repository object
            const repo = item.repository as unknown as { full_name: string; html_url: string };
    ...
    const concurrency = 6;
    let i = 0;
    const enriched: Map<string, RepoAgg> = new Map();
    async function worker() {
      while (true) {
        ...
        try {
          ...
          await maybeWaitOnRateLimit(res.headers as any);
          if (res.status === 200) {
          ...
          const status = err?.status ?? err?.response?.status;
          if (status === 404) continue; // repo gone
          const headers = (err?.response?.headers ?? {}) as Record<string, string>;
          const retryAfter = Number((headers as any)['retry-after'] ?? 0);
          const reset = Number((headers as any)['x-ratelimit-reset'] ?? 0);
          if (status === 403 || status === 429) {
  ...
  // Merge and sort
  const merged: CsvRow[] = (existingRows.map((r) => ({
    type: r.type ?? '',
    description: (r.description ?? '').replace(/\r/g, ' ').replace(/\n/g, ' '),
    link: normalizeRepoUrl(r.link),
    depth: Number.isFinite(r.depth) ? (r.depth as number) : cfg.defaultDepth,
  })) as CsvRow[]).concat(newRows);
