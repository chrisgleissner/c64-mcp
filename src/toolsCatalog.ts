...
export class ToolsCatalog {
  ...
  @McpTool({ name: 'info', description: 'Get device information' })
  info() {}

  @McpTool({ name: 'basic_v2_spec', description: 'Get Commodore BASIC v2 concise language spec or filtered by topic', parameters: { topic: 'string' } })
  basic_v2_spec() {}

  @McpTool({ name: 'asm_quick_reference', description: 'Get the 6502/6510 assembly quick reference or filter by topic keywords', parameters: { topic: 'string' } })
  asm_quick_reference() {}

  @McpTool({ name: 'vic_ii_spec', description: 'Get the VIC-II timing/graphics spec or filter by topic', parameters: { topic: 'string' } })
  vic_ii_spec() {}

  @McpTool({ name: 'file_info', description: 'Inspect file metadata', parameters: { path: 'string' } })
  file_info() {}

  @McpTool({ name: 'record_and_analyze_audio', description: 'Records audio from default microphone and analyzes SID playback, returning SIDWAVE-compatible pitch/frequency data', parameters: { durationSeconds: 'number', expectedSidwave: 'object' } })
  record_and_analyze_audio() {}

  @McpTool({ name: 'analyze_audio', description: "Smart audio verification tool that automatically analyzes audio when user requests to check, verify, or test music/SID playback. Responds to natural language patterns like 'check the music', 'verify the song', 'does it sound right?'", parameters: { request: 'string', durationSeconds: 'number', expectedSidwave: 'object' } })
  analyze_audio() {}

  @McpTool({ name: 'music_generate', description: 'Generate and play a simple arpeggio pattern', parameters: { root: 'string', pattern: 'string', steps: 'number', tempoMs: 'number', waveform: 'string' } })
  music_generate() {}
  ...
  @McpTool({ name: 'sid_reference', description: 'Return the SID overview document' })
  sid_reference() {}

  @McpTool({ name: 'sid_file_structure', description: 'Return the SID file structure overview' })
  sid_file_structure() {}

  @McpTool({ name: 'printing_commodore_text', description: 'Return Commodore MPS text printing guide' })
  printing_commodore_text() {}

  @McpTool({ name: 'printing_commodore_bitmap', description: 'Return Commodore MPS bitmap/custom char guide' })
  printing_commodore_bitmap() {}

  @McpTool({ name: 'printing_epson_text', description: 'Return Epson FX text printing guide' })
  printing_epson_text() {}

  @McpTool({ name: 'printing_epson_bitmap', description: 'Return Epson FX bitmap guide' })
  printing_epson_bitmap() {}

  @McpTool({ name: 'printing_prompts', description: 'Return end-user printing prompt templates' })
  printing_prompts() {}

  @McpTool({ name: 'printing_guide', description: 'Return the Commodore/Epson printing guide' })
  printing_guide() {}

  @McpTool({ name: 'rag_retrieve_basic', description: 'Retrieve BASIC references for a query', parameters: { q: 'string', k: 'number' } })
  rag_retrieve_basic() {}
